...
class ProductManagerMultiUser:
    def __init__(self, user_data):
        self.user_data = user_data
        self.username = user_data[1]  # username
        self.full_name = user_data[2]  # full_name
        self.location = user_data[3]  # location
        self.business_name = user_data[4]  # business_name
        self.usd_to_dzd_rate = user_data[5]  # usd_to_dzd_rate
        
        self.root = tk.Tk()
        self.root.title(f"Product Management - {self.username}")
    ...
    def init_database(self):
        """Initialize SQLite database for storing products (user-specific)"""
        db_filename = f'products_{self.username}.db'
    ...
    def create_widgets(self):
        ...
        title_text = "Product Management System"
        if self.business_name:
            title_text += f" - {self.business_name}"
        ...
        if self.full_name:
            user_info += f" ({self.full_name})"
        if self.location:
            user_info += f" | Location: {self.location}"
        user_info += f" | USD Rate: {self.usd_to_dzd_rate}"
        
        user_label = tk.Label(title_frame, text=user_info, 
        ...
        user_menu.add_command(label="Export Data", command=self.export_data)
    ...
    def create_product_tab(self):
        ...
        ttk.Label(form_frame, text=f"Cost Price (DZD):").grid(row=3, column=0, sticky=tk.W, pady=2)
    ...
    def export_data(self):
        ...
        if filename:
            try:
                import shutil
                db_filename = f'products_{self.username}.db'
                shutil.copy2(db_filename, filename)
                messagebox.showinfo("Success", f"Data exported to {filename}")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to export data: {str(e)}")
    
    # Copy all the methods from the original ProductManager class
    # (calculate_dzd, browse_picture, browse_package_image, add_product, etc.)
    
    def calculate_dzd(self, event=None):
        """Calculate DZD price from USD using user's exchange rate"""
        try:
...
if __name__ == "__main__":
    # This shouldn't be called directly - use user_manager.py instead
    print("Please run user_manager.py to select a user first!")